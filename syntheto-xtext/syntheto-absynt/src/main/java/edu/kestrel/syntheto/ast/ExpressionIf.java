...
/**
 * Syntheto if-then-else expressions.
 */
public class ExpressionIf extends Expression {
    ...
    private ExpressionIf(Expression test, Expression then, Expression else_) {
        this.test = test;
        this.then = then;
        this.else_ = else_;
    }

    public static ExpressionIf make(Expression test,
                                    Expression then,
                                    Expression else_) {
        return new ExpressionIf(test, then, else_);
    }

    /**
     * The symbol for the ACL2 function that creates an instance of this class.
     */
    public static final SExpressionSymbol classMakerFn = SExpression.syntheto("MAKE-EXPRESSION-IF");

    /**
     * Constructs an If expression from an S-Expression
     * that looks like
     * (SYNTHETO::MAKE-EXPRESSION-IF :TEST testExpression :THEN thenExpression :ELSE elseExpression)
     *
     * @param buildFormArg The AST maker form.
     * @throws IllegalArgumentException If the list is malformed.
     */
    public static ExpressionIf fromSExpression(SExpression buildFormArg) {
    ...
    public String toString(int indentLevel) {
        ...
        s.append(line(indentLevel, "ExpressionIf {"));
